...
if [ "$MODE" = "site" ]; then
  ...
  # ── обязательный шаг на стороне бота «Бедолага» ─────────────────────────────
  # С их версии 4.11 регистрация и вход ограничены ПО АДРЕСУ посетителя. Бот
  # видит один-единственный адрес — адаптера, поэтому без этой настройки его
  # лимиты ложатся на всех сразу: тридцать регистраций в сутки на весь кабинет
  # и десять попыток входа в минуту на всех. Адрес посетителя адаптер уже
  # передаёт, боту остаётся начать ему верить. Сделать это за оператора нельзя:
  # бот может стоять на другой машине, и его .env нам недоступен.
  if [ "$BACKEND_CHOICE" = bedolaga ]; then
    ADP_ADDR="$(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}} {{end}}' \
                  "$(getval ADAPTER_CONTAINER)" 2>/dev/null | awk '{print $1}')"
    say ""
    say "  ${YLW}${BOLD}ОБЯЗАТЕЛЬНО в .env бота «Бедолага»:${RST}"
    say "    ${BOLD}CABINET_TRUSTED_PROXIES=${ADP_ADDR:-<адрес, с которого адаптер приходит к боту>}${RST}"
    say "    ${DIM}и перезапустить бота (docker compose up -d --force-recreate bot).${RST}"
    say "    ${DIM}Без этого бот считает ВСЕХ посетителей кабинета за один адрес: с версии 4.11${RST}"
