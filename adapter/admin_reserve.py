...
def _mode(defs: dict[str, dict[str, Any]]) -> str:
    ...
    Регистр приводим к нижнему, а вот смысл не трогаем. Их справочник хранит
    значение ровно таким, каким его прислали, поэтому в базе законно лежит «TRUE»
    — их рантайм разбирает это как «включено», а мы при сыром сравнении считали бы
    режим неизвестным: тумблер показал бы «выключено», а проверки включённого
    состояния не сработали бы вовсе.
    """
    return (_text(defs, _KEY_MODE, "false") or "false").strip().lower()
...
async def _runtime_mode(ctx: Ctx) -> str:
    """Режим грейса, в котором РАБОТАЕТ процесс бота. Пусто — узнать не удалось.

    ЗАЧЕМ ОТДЕЛЬНО ОТ СОХРАНЁННОГО. Запись в их справочник настроек применяется к
    живому объекту сразу, а `grace_access_runtime` читает режим ОДИН РАЗ на старте
    (`start()` кладёт `self._mode`). Значит состояние «сохранено Включён, работает
    Выключен» — обычное дело, и в нём наши пред-проверки смотрели не туда: гейт
    `if mode == "true"` пропускал и пустой сквад, и 0 ГБ, потому что судил по
    сохранённому значению. С 0 ГБ это не мелочь — человек переезжает на резервный
    сквад с нулём доступного трафика.

    Ручка появилась у них только в v4.11; на старых сборках её нет, и тогда
    остаётся прежнее поведение (судим по сохранённому) — это не хуже, чем было.
    """
    # soft=True глотает сбои сети и 5xx, но НЕ 401/403 — «чужая сессия» это не
    # «нет данных». Здесь же любой отказ обязан означать ровно одно: рабочий режим
    # узнать не удалось, работаем как раньше. Без этого нового похода в бота
    # хватало, чтобы уронить СОХРАНЕНИЕ всего раздела там, где раньше оно
    # работало — то есть починка одного была бы поломкой другого.
    # Свой короткий предел: это ВСЕГО ЛИШЬ подсказка, а общий дедлайн обработчика
    # 30 секунд. Зависшая (не отвечающая ошибкой, а именно висящая) ручка съела бы
    # почти весь дедлайн ДО первой записи — и сохранение настроек, которое раньше
    # проходило, обрывалось бы на медленном боте.
    try:
        data = await ctx.json(
            "/cabinet/admin/grace-access", {}, soft=True, timeout=httpx.Timeout(3.0)
        )
    except Exception:  # noqa: BLE001 — включая UpstreamError с их 401/403
        return ""
    # Форму ответа не предполагаем: другой форк бота или посторонний прокси на этом
    # пути могут вернуть 200 со списком или строкой, и `.get` упал бы AttributeError
    # уже ВНЕ try — то есть уронил бы сохранение раздела.
    if not isinstance(data, dict):
        return ""
    runtime = data.get("runtime") if isinstance(data.get("runtime"), dict) else data
    for key in ("running_mode", "runningMode"):
        value = str((runtime or {}).get(key) or "").strip().lower()
        if value:
            return value
    return ""
...
async def reserve_get(ctx: Ctx) -> dict[str, Any]:
    view = _view(await _load(ctx))
    # Рабочий режим показываем РЯДОМ с сохранённым. Без этого экран рисовал
    # «Выключен» там, где резерв фактически работает (их запись применяется сразу,
    # а рантайм читает режим один раз на старте), и админ получал на сохранение
    # непонятный отказ «задайте резерв не меньше 1 ГБ» — при выключенном, на его
    # взгляд, резерве.
    running = await _runtime_mode(ctx)
    if running:
        view["running_mode"] = running
        view["restart_required"] = running != view["mode"]
    return view
...
def _plan(payload: dict[str, Any], view: dict[str, Any], running: str = "") -> dict[str, Any]:
    ...
    # --- режим -------------------------------------------------------------
    mode = view["mode"]
    # «Живой» резерв — true в ЛЮБОМ из двух источников: сохранённом режиме и режиме
    # процесса. Их запись в справочник применяется к живому объекту настроек сразу,
    # а `grace_access_runtime` читает режим один раз на старте, поэтому расхождение
    # «сохранено одно, работает другое» — обычное дело, и судить только по
    # сохранённому значит быть слепым ровно там, где резерв реально выдаётся.
    #
    # ДВА СОСТОЯНИЯ, А НЕ ОДНО, и путать их нельзя. `live_now` — что происходит
    # СЕЙЧАС: по нему судим о прыжке «Включён → Выключен», он опасен именно тем, что
    # кто-то УЖЕ висит на резервном скваде. А рабочей ли получится настройка,
    # проверяем в конце по ИТОГОВОМУ режиму — `is_live(mode)` уже после присвоения
    # `mode = candidate`. Возьми одно вместо другого — и включение off→true
    # перестанет проверяться вовсе.
    def is_live(saved: str) -> bool:
        return saved == "true" or running == "true"

    live_now = is_live(mode)
    if (value := _changed(payload, "mode", mode)) is not None:
        ...
        # По «живому» режиму, а не только по сохранённому: в расхождении «сохранено
        # Наблюдение, процесс работает Включён» прежняя проверка пропускала переход
        # — и люди, которым резерв уже выдан, оставались висеть на резервном скваде.
        #
        # НО «СЛИВ» ИСКЛЮЧАЕМ, И ЭТО НЕ ПОБЛАЖКА. «Слив» — и есть тот самый шаг, к
        # которому отсылает текст ниже: новым резерв не выдаётся, выданный
        # доводится до конца. Админ, который его уже сделал, обязан иметь
        # возможность выключить резерв — иначе отказ советует то, что человек
        # только что выполнил, и выхода не остаётся вовсе. (Первый заход этой
        # правки именно так и заблокировал штатное выключение.)
        if live_now and mode != "drain" and candidate == "false":
            raise _bad_request(
    ...
    #
    # СУДИМ ПО ЖИВОМУ РЕЖИМУ, А НЕ ТОЛЬКО ПО СОХРАНЁННОМУ. Их запись в справочник
    # применяется к живому объекту настроек сразу, а рантайм грейса читает режим
    # один раз на старте. В расхождении «сохранено Выключен, работает Включён»
    # прежний гейт пропускал и пустой сквад, и 0 ГБ — и человек переезжал на
    # резервный сквад с нулём доступного трафика. Поэтому грейс считается живым,
    # если так говорит ЛЮБОЙ из двух источников.
    #
    # Сравниваем разобранные значения, а не сырые строки: их собственный
    # справочник хранит значение как прислали, поэтому в базе законно лежит и
    # «TRUE» — сырое сравнение такую настройку пропустило бы мимо проверок.
    if is_live(mode):
        if gb < 1:
            raise _bad_request(
...
async def reserve_put(ctx: Ctx) -> dict[str, Any]:
    ...
    changes = _plan(payload, view, await _runtime_mode(ctx))
