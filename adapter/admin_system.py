...
async def _cabinet_block(force: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "commands": [
            {
                "cmd": "cd /opt/remnashop && ./update.sh --cabinet-only",
                "when": (
                    "Кабинет стоит на сервере бота (обычная установка). Каталог — тот, "
                    "куда его ставили. Обновятся кабинет и этот адаптер; наш бот на том же "
                    "сервере и его база не трогаются. Без флага скрипт сам спросит, "
                    "обновлять ли заодно и наш бот."
                ),
            },
            {
                ...
                "when": (
                    ...
                    "повторный запуск того же установщика; update.sh там не обновляет, а "
                    "останавливается и подсказывает эту команду."
                ),
            },
        ],
        "items": items,
