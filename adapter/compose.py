...
FEATURE_REQUIREMENTS: dict[str, list[tuple[str, str]]] = {
    ...
    # отфильтрованным. Поверх «Бедолаги» не строим (решение владельца): срок у них
    # меняется своими механиками, а сообщения — своими рассылками. Ключ всё равно
    # нужен: без него кабинет посчитал бы пункты доступными (нет ключа = умеет) и
    # показал бы их чужому админу. Ручек у адаптера нет — пункты и панель задач
    # не рисуются и ни одного запроса к этим путям не уходит.
    "bulk_days": [("GET", "/api/admin/users/bulk/days/preview")],
    "bulk_message": [("GET", "/api/admin/users/bulk/message/preview")],
}
...
FEATURE_OVERRIDES: dict[str, bool] = {
    # «Нужно больше устройств?» поверх «Бедолаги» не строим (решение владельца):
    # у них своя докупка устройств, `device_limit` в витрине и устройствах уже с
    # докупленными, а смена тарифа устроена иначе. Витрина и устройства при этом
    # обслуживаются — без явного false кабинет посчитал бы блок доступным и пошёл
    # бы за /offers с Главной и «Устройств».
    # Переводы «Информации» — наши: тексты лежат в нашем assets/info_content.json,
    # а поверх «Бедолаги» раздел обслуживает ИХ CMS, и параметр языка она не знает —
    # сохранение перевода ушло бы в их язык по умолчанию и затёрло бы основной текст.
    "info_i18n": False,
    "device_upsell": False,
    # Докупка «+1 устройства» — тоже наша, и тоже не поверх «Бедолаги»: там докупка
    # своя (своя цена, свой минимум, своё списание с баланса), а `device_limit` в их
    # ответах уже включает докупленные места. Без явного false кабинет посчитал бы
    # функцию доступной («нет ключа = умеет») и пошёл бы за несуществующей ручкой.
    "extra_device": False,
    # Докупка ТРАФИКА — тоже наша. У «Бедолаги» лимит трафика живёт по своим правилам
    # (свои сбросы, свой учёт), наших таблиц extra_traffic_* там нет вовсе, и без
    # явного false кабинет посчитал бы функцию доступной («нет ключа = умеет») и
    # пошёл бы за несуществующей ручкой /subscription/extra-traffic.
    "extra_traffic": False,
    # Напоминание о незавершённой оплате — наша фича: счета «Бедолаги» живут в её
    # базе, мы их не видим и напоминать по ним не можем.
    "payment_reminder": False,
    # «Сигналы до ухода» — тоже наши: вопрос «Всё работает?» и «давно не был» шлёт
    # наш крон по нашим подпискам, пользователи «Бедолаги» живут в её базе.
    "churn_signals": False,
}
