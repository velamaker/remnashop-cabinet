...
CASES: list[Case] = [
    ...
    Case(
        ...
        fields={
            ...
            # Что из нужного кабинету умеет НАШ бот (токены). Нет поля — бот старше
            # 1.4.0: кабинет прячет функции из манифеста (cabinet/src/lib/botCapabilities.ts).
            # Чужой бэкенд поле не шлёт — за него решает `features`.
            "bot_capabilities": F("list", req=False, why="какие функции кабинета прятать до обновления бота"),
        },
    ),
    Case(
        "/plans/public", "public",
