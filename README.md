...
### 🖥 Железо для ОТДЕЛЬНОГО сайт-сервера (сценарий Б)
...
| CPU | 2 vCPU |
| Диск | 15–20 ГБ |
| Канал | по числу юзеров кабинета (это API-трафик, не VPN) |

> Сам **VPN-трафик через этот сервер НЕ идёт** — он только отдаёт кабинет и
> проксирует API, поэтому мощный канал/CPU не нужны.
