...
Обновляетесь на той же машине, где ставили. Настройки (`.env`, оформление, тарифы, данные) **не теряются**.

**Сервер бота** (сценарий А, или шаг 1 сценария Б) — обновить кабинет и админку:
...
<details>
<summary>Обновить сам образ бота RemnaShop</summary>

Наш код и образ бота обновляются раздельно. Обычный `docker compose pull` образ бота **не** обновит — он собирается локально. Чтобы поднять версию базового бота, укажите тег:

```bash
cd /opt/remnashop
./update.sh --base <тег>     # например: ./update.sh --base v0.8.2
```

Сообщение «нет обновления» относится именно к базовому образу.
