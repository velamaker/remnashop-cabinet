...
- Если **443 занят** (у вас уже свой reverse-proxy) — скрипт его не трогает и просто
  показывает готовый блок (Caddy и nginx) для вставки на `127.0.0.1:5002`.
...
> `assets/menu.json`. Цвет `null` = взять из темы; название по умолчанию — имя
> Telegram-бота.
