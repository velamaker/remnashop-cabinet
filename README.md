...
**Как обновиться?**
На сервере с ботом — `./update.sh` (бэкап БД → `git pull` → пересборка → логи).
На отдельном сайт-сервере — `git pull && ./install.sh site`.

> ⚠️ Авторская команда `docker compose pull && down && up` нашему форку **не
> подходит**: overlay бота собирается локально (`--build`), базовый образ
> запиннен, а кабинет — в отдельном compose-файле. Уведомление «нет обновления»
> относится к базовому образу, а не к коду форка.
