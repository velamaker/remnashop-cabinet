...
**Публикация — автоматическая.** Установщик сам подхватит Caddy панели Remnawave или ваш Caddy/nginx, а если прокси нет — поставит выбранный и выпустит сертификат. От вас — только A-запись `cabinet.example.com` → IP сервера (порты `80`/`443` открыты).
...
**HTTPS — ставится само.** Если `443` свободен — установщик спросит, чем поднять (Caddy или nginx), поставит выбранный и выпустит сертификат сам. Если `443` занят вашим прокси — не трогает его и подскажет направить на `127.0.0.1:5002`.

> Неинтерактивно задаётся заранее: `HTTPS=caddy`, `HTTPS=nginx` или `HTTPS=none`. Для nginx нужен валидный DNS на момент установки — иначе certbot выпустит сертификат позже одной командой, которую покажет скрипт.
