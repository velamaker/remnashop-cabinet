...
**Что нужно заранее:** настроенный и работающий бот RemnaShop (его `.env`
заполнен). Docker на таком сервере уже установлен.

**Одной командой** (загружает код в каталог бота и запускает установку; Docker
не устанавливает — он уже есть):

```bash
curl -fsSL https://raw.githubusercontent.com/alexdsndr161rus2015-maker/remnashop-cabinet/test/bot-install.sh | sudo bash
```

> Если `.env` ещё не заполнен, скрипт создаст его из примера и попросит указать
> секреты бота (`BOT_TOKEN`, `REMNAWAVE_*`, `APP_DOMAIN` и т.д.); после этого
...
**После установки** опубликуйте наружу **только кабинет** (порт `5002`) — бот/API
уже выведен вашим существующим reverse-proxy. Добавьте блок с TLS:
...
Кабинет на отдельной машине от бота. Он проксирует `/api/` на **публичный API
бота** (который уже доступен через reverse-proxy бота на его домене) — поэтому
**WireGuard/VPN не требуется**, это обычное проксирование по HTTPS. Браузер
обращается только к домену кабинета, а nginx проксирует `/api/` внутрь
(same-origin), так что cookie и CORS работают корректно. Дополнительного риска
нет: API бота и без того доступен извне и защищён авторизацией.

> ⚠️ **Сначала — на сервере БОТА.** Кабинет обращается к эндпоинтам
> `/api/v1/public/*` и `/api/v1/admin/*`, которых у базового бота нет — их
> добавляет overlay этого репозитория. Поэтому на сервере бота должен быть
> установлен RemnaShop-overlay (см. [Сценарий А](#️-сценарий-а--на-сервере-с-ботом),
> команда `bot-install.sh`). Если бот уже работает с кабинетом — доустанавливать
> ничего не нужно. Итог: «отдельный сервер» = **сервер бота (overlay) + сервер
> кабинета**.

**На отдельном сервере — одной командой** (устанавливает Docker, Caddy, код,
кабинет и TLS-прокси на 443; запросит 3 значения):
