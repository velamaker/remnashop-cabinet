...
**Что сделать после (1 раз):** опубликуйте кабинет наружу через **ваш уже
работающий** reverse-proxy (тот же Caddy/nginx, что отдаёт панель/бота). Добавьте
в его конфиг блок для домена кабинета — он указывает на `127.0.0.1:5002`:
...
> Свой Caddy в этом сценарии скрипт **не ставит** — вы пользуетесь тем, что уже
> есть. Не забудьте A-запись `cabinet.example.com` → IP сервера.
