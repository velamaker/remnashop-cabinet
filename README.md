...
> дублирует блок. Если Caddy панели не найден — установщик подскажет блок для
> вашего прокси (`cabinet.example.com { reverse_proxy 127.0.0.1:5002 }`).
...
**Про HTTPS (Caddy — на ваш выбор):** в конце скрипт спросит, поставить ли Caddy
и автоматически выпустить TLS на `443`.
- Если порт **443 свободен** — можно согласиться, и сертификат выпустится сам.
- Если у вас **свой reverse-proxy** (nginx или Caddy-контейнер панели) — откажитесь:
  скрипт покажет готовый блок, который вы добавите в свой прокси на
  `127.0.0.1:5002`. Так свой Caddy **не займёт 443** и не сломает webhook'и панели.

> Если 443 уже занят, скрипт сам определит это и Caddy ставить не станет —
> просто покажет блок для вашего прокси.

---

### 🔑 Обязательный шаг — вход через Telegram (`/setdomain` в @BotFather)

Касается **обоих сценариев**. Чтобы работала кнопка **«Войти через Telegram» в
браузере**, домен кабинета нужно один раз привязать к боту:

1. Откройте [@BotFather](https://t.me/BotFather) → команда `/setdomain`.
2. Выберите своего бота.
3. Отправьте **домен кабинета без `https://`** — например `cabinet.example.com`.

> Без этого шага вход через Telegram **в браузере** не сработает (Telegram не
> отдаст данные на непривязанный домен). Внутри Telegram как **Mini App** вход
> работает и так — там используется `initData`, привязка домена не нужна.
> Скрипт этот шаг не делает — его нельзя автоматизировать, он на стороне Telegram.
