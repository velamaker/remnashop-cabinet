...
Что спросит дополнительно: адрес API бота и — один раз — его токен
(строка `WEB_API_DEFAULT_TOKEN` из `.env` бота Bedolaga). Токен нужен только для
заморозки подписки; без него кабинет работает, просто без неё.
...
- **Разделы, которых у бота нет**, не показываются мёртвыми кнопками — они скрыты.
  Пополнение баланса, оплата с баланса и автопродление работают; покупка тарифа
  картой одной кнопкой пока выключена намеренно — у этого бота её нет, покупают
  с баланса: [docs/BEDOLAGA-MONEY.md](docs/BEDOLAGA-MONEY.md).
...
- **Проверить после установки:** установщик сам печатает ответ самопроверки —
  видит ли переводчик бота, панель и свой токен. Подробная проверка (какие экраны
  кабинета оживут) — `adapter/selfcheck.py`, команда в
  [docs/BEDOLAGA-MAPPING.md](docs/BEDOLAGA-MAPPING.md).
