...
export interface Appearance {
  ...
  // Вход по почте включён на бэкенде. Отсутствует — считаем, что включён:
  // молча спрятать рабочую форму хуже, чем показать лишнюю. Поверх чужого бота
  // оператор может выключить этот способ целиком, и тогда кабинет обязан не
  // предлагать то, что заведомо ответит отказом.
  email_auth_enabled?: boolean;
  // Согласие с документами при регистрации. Если бэкенд его требует, а кабинет
  // не пришлёт отметки, аккаунт не создастся вовсе.
  legal_consent_required?: boolean;
  legal_consent_prechecked?: boolean;
  // Только КЛЮЧИ документов («public_offer», «privacy_policy»): подписи к ним
  // кабинет берёт из своего словаря, потому что бэкенд их не отдаёт.
  ...
  // Что из нужного кабинету умеет НАШ бот (токены, см. lib/botCapabilities). Нет
  // поля — бот 1.3.8 или старше: функции из манифеста прячутся, прочее как было.
