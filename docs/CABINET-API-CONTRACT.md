...
| GET | `/api/subscription/extra-traffic` | Докупка «+N ГБ к ТЕКУЩЕМУ периоду трафика»: объём, фиксированная цена, момент обновления трафика и чем платить. Продажи закрыты — ответ ровно `{enabled:false}`, цену наружу не отдаём. Панель читается один раз и только для тех, кому вообще можно продать | {enabled:false} \| {enabled:true, currency, currency_symbol, gb, price, balance, traffic_limit_gb, plan_traffic_limit_gb, used_bytes (null — панель не ответила), extra_gb_active, strategy, resets_at (момент обновления трафика по правилам панели; null — стратегия без обновления), show_from_percent (с какого расхода кабинет показывает предложение), subscription_expire_at, gateways[{gateway_type,currency_symbol}], offer{available,reason,until,hours_left}}. Коды reason: disabled, no_subscription, trial, unlimited_traffic (безлимит в строке ИЛИ в тарифе), unlimited_term, frozen, reserve, not_active (LIMITED при этом ПРОХОДИТ — это главный покупатель), reset_unknown (нет даты создания в панели, срок назвать нечем), window_cap (упор в потолок ГБ на период), reset_too_soon (до обновления меньше min_hours_left), panel_unavailable |
| POST | `/api/subscription/extra-traffic/buy` | Купить объём с ₽-баланса или выставить счёт через рублёвый шлюз. `request_id` — UUID кабинета на каждое подтверждение: повтор с тем же ключом отдаёт сохранённый результат, чужой ключ → 409. `expected_gb` и `expected_amount` сверяются с серверными: владелец мог поменять условия между показом и нажатием. Бизнес-отказы приходят как 200 с `result`, а не ошибкой HTTP | {request_id,pay:balance\|gateway,gateway_type?,expected_amount,expected_gb} → {result:applied,gb,traffic_limit_gb,unlocked (панель сама сняла LIMITED),until,spent,balance} \| {result:pending,payment_id,payment_url,amount} \| {result:price_changed,quote} \| {result:insufficient_balance,need,balance} \| {result:not_available,reason,quote}. Ошибки: 409 почта, 404 шлюз, 502 панель («деньги не списаны»), 503 шлюз или запись заказа |
...
| GET, PUT | `/api/admin/extra-traffic` | Докупка трафика: тумблер продаж (ВЫКЛ по умолчанию), объём за покупку, фиксированная цена (пусто — продажи закрыты даже при тумблере), минимум счёта, порог показа на Главной, минимум часов до обновления трафика, потолок ГБ на период, тумблеры уведомлений и возврат ₽ при отзыве. Плюс подсказка о шаге соседних тарифов, разбивка по стратегиям обновления и итоги за 30 дней | {config{enabled,gb_per_purchase,price_rub,min_amount_rub,show_from_percent,min_hours_left,max_gb_per_window,notify_users,notify_admins,notify_limited,refund_on_revoke}, effective_enabled, hint[{from_gb,to_gb,diff_30d_rub,device_diff}], strategies[{strategy,subscriptions}], short_window (есть DAY/WEEK — прибавка живёт меньше суток или недели), summary{applied_30d,gb_30d,amount_30d,rejected_30d,credited_open,active_grants,active_gb}} |
...
| GET, POST | `/api/admin/subscriptions/user/{id}/extra-traffic, /extra-traffic/{grant_id}/revoke` | Журнал докупок трафика и отзыв прибавки. Отзыв опускает лимит по правилу «тариф + оставшиеся прибавки» и по флагу возвращает ПОЛНУЮ сумму заказа на баланс (по умолчанию не возвращает: у объёма нет доли «непрожитого») | {grants[{id,subscription_id,status,gb,strategy,granted_at,ends_at,end_reason}], orders[{id,status,source,gb,amount,created_at,reason,grant_id}]}; revoke {refund} → {success,traffic_limit_gb,refunded}. PREVIEW-админу суммы заказов приходят null |
...
| GET, PUT | `/api/admin/info` | Редактирование текстов страницы «Информация». `?lang=xx` — перевод (пустое поле снимает перевод) | faq[], rules, privacy, offer, statuses, lang, own, base, translated_langs |
