...
| `device_upsell` | блок «Нужно больше устройств?» на Главной и в «Устройствах» | выключен явно (`FEATURE_OVERRIDES`), а не по путям: витрину и устройства адаптер отдаёт, но докупка устройств у них своя, `device_limit` уже с докупленными, смена тарифа устроена иначе. Страница оплаты при этом прежняя: условий смены их витрина не шлёт, и подтверждения «сгорят дни» нет |
| `bulk_days`, `bulk_message` | «Пользователи» → «Массово по фильтру»: «Добавить дни подписки…», «Написать сообщение…» и панель фоновых задач | поверх их бота не строим: срок и рассылки у них свои. Привязаны к путям предпросмотра (`/api/admin/users/bulk/*/preview`) — ручек нет, пунктов и запросов нет. Адаптер старее кабинета ключей не знает: пункт виден только полному доступу, первый же предпросмотр получает 501 и пункт прячется |
...
### Адрес посетителя: `CABINET_TRUSTED_PROXIES` — обязательный шаг у бота

С версии **4.11** «Бедолага» ограничивает вход и регистрацию **по адресу
посетителя**: 10 попыток входа в минуту и, сверх прежнего минутного окна,
часовое и суточное окно регистраций. Бот при этом видит один-единственный адрес —
адаптера. Значит без настройки его лимиты ложатся не на человека, а на **весь
кабинет разом**: один посетитель упирает в них всех остальных.

Адрес посетителя адаптер передаёт сам (`X-Forwarded-For` и `X-Real-IP` одним
значением). Чтобы бот начал ему верить, в его `.env` нужно:

```
CABINET_TRUSTED_PROXIES=<адрес или подсеть, с которой адаптер приходит к боту>
```

и перезапустить бота **пересозданием** контейнера (`docker compose up -d
--force-recreate bot`): обычный `restart` их `env_file` не перечитывает.
Проверить, что настройка доехала: `docker exec <бот> env | grep TRUSTED`.

**Откуда берётся значение.** Один сервер — адрес контейнера адаптера в общей сети
(его печатает установщик в конце установки). Отдельный сервер — публичный адрес
машины кабинета или подсеть фронт-прокси, если бот стоит за ним.

**Почему нельзя просто доверять заголовку.** Их `get_client_ip` берёт ПЕРВЫЙ
элемент `X-Forwarded-For` — но только от доверенного соседа. Адаптер шлёт туда
ровно одно значение и вырезает всё, что пришло от браузера
(`main._HOP_BY_HOP`), поэтому подделать его через кабинет нельзя. Само значение
адаптер берёт с доверенного конца цепочки — из `X-Real-IP`, который наш nginx
ставит из вычисленного `$remote_addr`, а не из начала `X-Forwarded-For`
(начало пишет браузер, и до сентября 2026 бралось именно оно — то есть адрес
подделывался одной строкой).
