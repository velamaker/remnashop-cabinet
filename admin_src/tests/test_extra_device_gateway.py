...
# Часы кода — на ту же дату, что и данные теста.
#
# ПОЧЕМУ ЭТО ОБЯЗАТЕЛЬНО. Данные здесь построены от фиксированного NOW, а код берёт
# настоящее время (`extra.now_utc`). Пока тест гоняли в день написания, это совпадало;
# на следующий день заказ «пятиминутной давности» становился старше суток
# (`STALE_HOURS`), ветка докупки отклоняла его, и три теста краснели на ровном месте —
# без единой правки кода. Тест, который живёт один день, хуже отсутствующего: он
# приучает не верить прогону.
