...
PLAN_CHANGE_CONFIRM_NEW = (
    ...
    "{ NUMBER($carry_lost, useGrouping: 0) } дн. перенести нельзя — цена прежних дней неизвестна.</i>\n"
    f"        [CAPPED] <i>{_WARN} Текущая подписка будет <u>заменена</u> выбранной. "
    "К новому сроку добавится { NUMBER($carry_bonus, useGrouping: 0) } дн. — это предел переноса, "
    "ещё { NUMBER($carry_lost, useGrouping: 0) } дн. не поместятся.</i>\n"
    f"        [NOPRICE] <i>{_WARN} Текущая подписка будет <u>заменена</u> выбранной. "
    "Остаток { NUMBER($carry_left, useGrouping: 0) } дн. перенести нельзя: у выбранного срока "
    "нет цены в этой валюте.</i>\n"
...
PLAN_CHANGE_SUCCESS_NEW = (
    ...
    "\n"
    "    <b>{ $plan_name }</b>\n"
    # Вариант по умолчанию — ПУСТОЙ и без чисел: если обёртка геттера не встала и
    # переменных нет, NUMBER() по отсутствующей переменной уронил бы итог покупки.
    "    { $carry_added ->\n"
    "        [YES] Остаток прежнего плана пересчитан: "
