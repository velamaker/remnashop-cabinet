...
(`web/endpoints/admin/gateways.py`), но статистика на флаг не смотрела. Экран
«Статистика ЮMoney» показывал 164 транзакции, из которых 24 — проверки.
...
НА БОЕВЫХ ДАННЫХ (15 сентября 2026) вторая часть оказалась больше первой:

    кто                транзакций   завершённых   учтено как выручка
    клиенты (USER)         147           73            25 113 ₽
    владелец (OWNER)        71           13            11 463 ₽
    служебная (DEV)          7            2                 4 ₽

По одному только ЮMoney «Общий доход» падает с 31 695 ₽ до 20 232 ₽: больше
трети показанной выручки была собственными деньгами владельца. У Heleket и
Telegram Stars показанный доход состоял ИЗ ОДНИХ проверок (4 ₽ и 2 ₽) — то есть
шлюз, через который не прошло ни рубля, выглядел работающим и зарабатывающим.
...
покупателю — и его прошлые покупки уйдут из статистики задним числом. Сегодня
терять нечего (у всех неклиентских учёток, кроме владельца, 4 ₽ на двоих), но
если такое понадобится — заводите человеку отдельный аккаунт, а не роль.
