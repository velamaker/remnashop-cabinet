"""Выдача SDK панели с учётом её версии (overlay).

ЗАЧЕМ ПЕРЕОПРЕДЕЛЁН. В Remnawave 3.0 сменился контракт пользователя: убран `uuid`,
всё адресуется числовым `id`, удалены точечные поиски, DELETE отвечает 204 без тела,
`ip-control` переименован в `connections`. Правка каждого вызывающего места означала бы
два расходящихся набора кода, поэтому решение принимается ОДИН РАЗ и в одной точке —
здесь: под 2.x отдаём ровно тот же `RemnawaveSDK`, что и раньше, под 3.x — обёртку
`RemnawaveSDKv3` с тем же интерфейсом (см. infrastructure/services/remnawave_v3.py).

ВЕТКУ ВЫБИРАЕТ ПАНЕЛЬ, А НЕ КОНФИГ. Спрашиваем `/api/system/metadata` — единственный
маршрут, который в 3.x не менялся и который отдаёт `response.version`. Никакого
тумблера в .env: он рано или поздно разъедется с реальностью после апгрейда панели.

ПАНЕЛЬ НЕДОСТУПНА — НЕ ПАДАЕМ. Панель перезагружается чаще, чем бот: если её недоступность
на старте будет ронять контейнер, бот уйдёт в рестарт-петлю и не поднимется даже тогда,
когда панель вернётся. Поэтому при неудачном опросе берём ветку 2.x (сегодняшнее
поведение) и пишем в лог причину. Проверка совместимости версии и без нас есть —
`RemnawaveImpl.try_connection` делает три попытки при старте и ругается отдельно.
"""
