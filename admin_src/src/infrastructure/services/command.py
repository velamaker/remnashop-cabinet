...
# OVERLAY: подписи команды /gift по языкам бота (ключ = код локали).
_GIFT_DESCRIPTIONS: dict[str, str] = {
...
class CommandService:
    ...
    async def setup_commands(self) -> None:
        ...
        for lang in locales:
            ...
            # OVERLAY: /gift в списке команд (☰ у поля ввода). В базовый enum Command
            # не лезем — иначе пришлось бы вендорить core/enums.py целиком и держать
            # его в синхроне при обновлении базы. Ключа в ftl нет, подписи здесь же:
            # неизвестный язык получает английскую.
            commands.append(
                BotCommand(
                    command="gift",
                    description=_GIFT_DESCRIPTIONS.get(
                        str(lang or self.config.default_locale), _GIFT_DESCRIPTIONS["en"]
                    ),
                )
            )
