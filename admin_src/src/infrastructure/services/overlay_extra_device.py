...
ЧТО ПРОДАЁМ. Место под устройство к ТЕКУЩЕЙ подписке ПЕРИОДОМ 30 ДНЕЙ — или короче,
если срок подписки кончается раньше (место её не переживает). Цена — «₽ за устройство
в 30 дней» пропорционально длине периода, вверх до рубля, не меньше минимума; значит
разовый платёж НИКОГДА не превышает цену за 30 дней. Кончился период — можно продлить
ещё на такой же, с конца текущего.
Место — строка журнала с концом; лимит в базе и в панели остаётся ОДНИМ числом
...
РЕШЕНИЯ ВЛАДЕЛЬЦА, они же — отличия от первой редакции плана:
  * место продаётся периодом 30 дней (или до конца подписки, если он ближе), цена по
    умолчанию 100 ₽ за 30 дней, минимум остатка подписки для покупки — 7 дней;
  * перед концом места человеку предлагаются ДВА действия в порядке «сначала тариф
    побольше, потом продлить место ещё на период»;
...
# 30 суток в секундах — и единица цены, и ДЛИНА ПЕРИОДА места. Ровно 30 × 86400, без
# календарных месяцев: ни цена, ни длина не должны зависеть от того, в каком месяце
...
def period_end_for(cov_start: datetime, expire_at: datetime) -> datetime:
    """Конец периода места: 30 дней от начала, но не дальше конца подписки.

    Место не переживает подписку — за дни, которых у человека уже нет, брать деньги
    нельзя. И наоборот: продавать «до конца срока» сразу целиком владелец не хочет —
    разовый платёж не должен быть больше цены за 30 дней.
    """
    return min(cov_start + timedelta(seconds=SEC30), expire_at)
...
def quote(
    ...
    """Цена за ПЕРИОД места. Считаем ДО СЕКУНДЫ и округляем вверх до рубля.
    ...
    на коротком периоде, и человек видит цену, не совпадающую с «осталось N дней».

    Период — 30 дней или остаток подписки, если он короче (см. period_end_for),
    поэтому цена одной покупки никогда не превышает `price_rub_30d`.
    """
    ...
    if kind == "extend":
        ...
        # Продление начинается с конца текущего места, а не «с сейчас»: иначе человек
        # платил бы второй раз за дни, которые уже оплатил.
        cov_start = slot.ends_at
    else:
        cov_start = now
    if st.expire_at is None:
        raise ValueError("у подписки нет срока")
    period_end = period_end_for(cov_start, st.expire_at)
...
def user_text(key: str, **kw: Any) -> str:
    ...
    if key == "applied":
        return (
            ...
            f"Место оплачено до {_dm(kw['until'])} — дальше его можно продлить."
        )
    if key == "not_applied":
        return (
    ...
    if key == "reminder":
        # ПОРЯДОК ДЕЙСТВИЙ — решение владельца: сначала тариф побольше (в нём, кроме
        # места, ещё и трафик, и он остаётся навсегда), и только потом продление места.
        # Конкретный тариф здесь не называем: витрина крону недоступна, а обещать
        # «тариф за N ₽» вслепую хуже, чем отправить на страницу, где видно цену.
        return (
            f"⏳ Оплаченное место под устройство работает до {_dm(kw['ends_at'])}, "
            f"а подписка — до {_dm(kw['until'])}.\n"
            "Что можно сделать, в кабинете:\n"
            "1. Перейти на тариф побольше — место останется с вами, а остаток "
            "текущего тарифа пересчитается в дни.\n"
            "2. Продлить место ещё на 30 дней — «Устройства» → «Продлить».\n"
            f"Если ничего не делать, лимит вернётся к {kw['limit']}, и последнее "
            f"добавленное устройство отключится {_dm(kw['ends_at'])}."
        )
    if key == "ended":
        removed = ""
        if kw.get("removed"):
            names = ", ".join(f"«{n}»" for n in kw["removed"] if n) or "последнее добавленное"
            removed = f"Отключили {names} — это последнее добавленное устройство. "
        return (
            f"Срок оплаченного места закончился — теперь можно подключить "
            f"{kw['limit']} устр. {removed}Нужно место снова: в кабинете можно перейти "
            "на тариф побольше или купить место ещё на 30 дней."
        )
    raise KeyError(key)
