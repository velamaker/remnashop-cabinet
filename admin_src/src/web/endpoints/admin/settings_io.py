...
CONFIG_FILES = [
    ...
    # Выключатель переноса остатка при смене тарифа: без него восстановленная
    # установка вернулась бы к дефолту (перенос включён), даже если владелец выключал.
    "plan_change.json",
    # Выключатель и цена докупки устройства: без строки восстановленная установка
    # вернулась бы к дефолту (продажи выключены), а цену владельца пришлось бы
    # вводить заново.
    "extra_device.json",
