...
DEFAULTS: dict[str, Any] = {
    ...
    # «Нужно больше устройств?»: при заполненном лимите устройств кабинет предлагает
    # ближайший тариф побольше. Никому не пишет и подписку не меняет — единственное
    # денежное действие дальше закрыто подтверждением на странице оплаты, поэтому
    # по умолчанию включено. Выключать, если HWID-лимит в панели отключён: тогда
    # «заняты все места» станет неправдой.
