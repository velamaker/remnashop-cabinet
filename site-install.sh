...
say ""
# Благодарность install.sh сказал бы посередине — между сборкой и HTTPS. Просим его
# промолчать: спасибо скажем сами, последней строкой установки.
RS_THANKS_LATER=1 bash install.sh site
...
# Благодарность и адреса для поддержки проекта (scripts/thanks.sh из только что
# скачанного кода). Этот же скрипт — и обновление кабинета на отдельном сервере,
# так что спасибо звучит и при установке, и при обновлении.
# Условием, а не телом: последняя команда скрипта — это его код выхода, и сбой
# подключения модуля не должен превращать прошедшую установку в «ошибку».
if [ -f scripts/thanks.sh ] && . scripts/thanks.sh; then thanks_banner README.md; fi
